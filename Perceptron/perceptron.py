...
df = pd.read_csv('Perceptron\optdigits.tra', sep=',', header=None) #reading the data by using Pandas library

X_training = np.array(df.values)[:,:64] #getting the first 64 fields to form the feature data for training
y_training = np.array(df.values)[:,-1]  #getting the last field to form the class label for training

df = pd.read_csv('Perceptron\optdigits.tes', sep=',', header=None) #reading the data by using Pandas library
